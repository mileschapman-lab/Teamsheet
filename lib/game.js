...
import { PLAYERS, BANK, DAILY_BANK } from "../data/players";
...
export function dailyPuzzleIndices(matchday, poolSize = (DAILY_BANK.length >= PER_DAY ? DAILY_BANK.length : BANK.length)) {
...
export function dailyPuzzles(matchday) {
  const pool = DAILY_BANK.length >= PER_DAY ? DAILY_BANK : BANK;
  return dailyPuzzleIndices(matchday).map((i) => pool[i % pool.length]);
}
